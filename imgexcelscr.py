import requests, shutil, xlsxwriter, os
...
def download_image(link_to_image, index):
    ...
    with open(f"book{index}.jpg", 'wb') as f:
...
file1 = get_excel_data(r'C:\Users\riley\Eco Warriors kids list April 2021- LOR.xlsx', 'ARTICLE TITLE')


# generates workbook and calls above functions to create an author's info in 1st row,
# and an image of the cover of the book in 2nd
def wbgen(book_data, filename):
    ...
    for i in range(1, len(book_data) +1):
        ...
        worksheet.insert_image(f'B{i}', f'book{i}.jpg') #writes the corresponding images it excel file
    workbook.close()
#final function call to write data from file1 into new excel file 'EcoWarriors_images'
wbgen(file1, 'EcoWarriors_Images')
